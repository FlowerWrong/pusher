...
func (s *Session) ParsePayload(payloadBytes []byte) error {
	...
	case PusherSubscribe:
		return s.subscribeHandler(payloadBytes)
	case PusherUnsubscribe:
		return s.unsubscribeHandler(payloadBytes)
	case PusherPing:
	...
	default:
		if IsClientEvent(requestEvent.Event) {
			return s.clientEventHandler(payloadBytes)
		}
...
func (s *Session) clientEventHandler(payloadBytes []byte) error {
...
func (s *Session) unsubscribeHandler(payloadBytes []byte) error {
...
func (s *Session) subscribeHandler(payloadBytes []byte) error {
